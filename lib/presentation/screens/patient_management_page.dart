...
class _PatientManagementPageState extends State<PatientManagementPage> {
  ...
  void _showPatientForm({DocumentSnapshot<Map<String, dynamic>>? doc}) {
    ...
    // Default to 'Not Specified' if data is missing to match the SignUpPage logic
              ...
              child: ListView(
                ...
                children: [
                  ...
                  DropdownButtonFormField<String>(
                    // Safety check: ensure 'value' exists in the 'items' list
                    value: ['Male', 'Female', 'Not Specified'].contains(gender)
                        ? gender
                        : 'Not Specified',
                    items: ['Male', 'Female', 'Not Specified']
                        ...
                        .toList(),
                    onChanged: (v) =>
                        setDialogState(() => gender = v ?? 'Not Specified'),
                    decoration: const InputDecoration(labelText: 'Gender'),
                  ),
                  DropdownButtonFormField<String>(
                    value: bloodTypes.contains(bloodType) ? bloodType : 'A+',
                    items: bloodTypes
                        .map((bt) =>
                        DropdownMenuItem(value: bt, child: Text(bt)))
                        .toList(),
                    onChanged: (v) =>
                        setDialogState(() => bloodType = v ?? 'A+'),
  ...
  void _showItemForm({
    ...
    final formKey = GlobalKey<FormState>();
    final docSnapshot = await patientsCollection.doc(patientId).get();
          ...
          ElevatedButton(
            onPressed: () async {
              if (formKey.currentState!.validate()) {
                List<dynamic> listKey = List.from(type == 'Lab'
      ...
      MaterialPageRoute(
        builder: (context) =>
            StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
              stream: patientsCollection.doc(patientId).snapshots(),
              builder: (context, snapshot) {
                ...
                final displayLabs = (data['labResults'] as List?)
                    ?.map((e) => Map<String, dynamic>.from(e))
                    .toList() ??
                    [];
                final displayRx = (data['prescriptions'] as List?)
                    ?.map((e) => Map<String, dynamic>.from(e))
                    .toList() ??
                    [];
                ...
                return Scaffold(
                  appBar:
                  AppBar(title: Text(data['fullName'] ?? data['name'] ?? '')),
                  body: ListView(
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            children: [
                              ...
                              Text(
                                  '${data['age']} yrs • ${data['gender']} • Blood: ${data['bloodType'] ?? 'N/A'} • ${data['condition']}'),
                              const SizedBox(height: 10),
                              Container(
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          IconButton(
                            ...
                            onPressed: () =>
                                _showItemForm(type: 'Lab', patientId: patientId),
                          )
                        ],
                      ),
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          IconButton(
                            icon: const Icon(Icons.add_circle, color: Colors.blue),
                            onPressed: () =>
                                _showItemForm(type: 'Rx', patientId: patientId),
                          )
                        ],
                      ),
                              ...
                              DataCell(Row(
                                children: [
                                  IconButton(
                                    ...
                                    tooltip: 'View',
                                    onPressed: () =>
                                        _openPatientDetails(patientDoc.id),
                                    splashRadius: 22,
                                  ),
                                  IconButton(
                                    icon: const Icon(Icons.edit,
